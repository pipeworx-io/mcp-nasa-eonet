...
`tools/list` at `https://gateway.pipeworx.io/nasa-eonet/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/nasa_eonet_events \
  -H 'Content-Type: application/json' \
  -d '{"status":"open","days":30,"limit":50}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/nasa_eonet_events`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
